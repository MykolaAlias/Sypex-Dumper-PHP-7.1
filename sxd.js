...
var sxd = {
    ...
    init: function () {
        ...
        this.timer.t1 = z('sxdTime1');
        this.timer.t2 = z('sxdTime2');
        this.timeout = null;
        this.log = zGridObject();
        this.log.init('sxdGrid1', {header: [[this.lng('dt'), 120], [this.lng('action')]], width: 565, height: 286});
        this.files = zGridObject();
        this.files.init('sxdGrid2', {
    ...
    openTab: function (name) {
        ...
        document.title = sxd.version.full;
    },
    addTabs: function (o) {
    ...
    comment: {},
    lng: function (s) {
        return sxdlng[s] ? sxdlng[s] : '[LNG: ' + s + ']';
    },
    timer: {
    ...
    actions: {
        db: function () {
            ...
            if (this.name == 'backup_db' || this.name == 'services_db')sxd.ajax('index', sxd.lng('load'), {
                act: 'load_db',
                name: this.name,
                value: this.value
            });
        }, dblist: function () {
            sxd.ajax('index', sxd.lng('load'), {act: 'dblist'});
        }, filelist: function () {
            sxd.ajax('index', sxd.lng('load'), {act: 'filelist'});
        }, files: function () {
            if (!this.value)return;
            sxd.ajax('index', sxd.lng('load'), {act: 'load_files', name: this.name, value: this.value});
        }, tab_connects: function () {
            sxd.ajax('index', sxd.lng('load'), {act: 'load_connect'});
            sxd.showDialog('connect');
        }, tab_createdb: function () {
            sxd.showDialog('createdb');
        }, tab_backup: function () {
            sxd.openTab('backup');
            if (sxd.tree.backup.load)sxd.combos.backup_db.action();
        }, tab_restore: function () {
            sxd.openTab('restore');
            if (sxd.tree.restore.load)sxd.combos.restore_file.action();
        }, tab_files: function () {
            sxd.ajax('index', sxd.lng('load'), {act: 'load_files_ext'});
            sxd.openTab('files');
        }, tab_services: function () {
            sxd.openTab('services');
            if (sxd.tree.services.load)sxd.combos.services_db.action();
        }, tab_options: function () {
            sxd.ajax('index', sxd.lng('load'), {act: 'load_options'});
            sxd.openTab('options');
        }, tab_exit: function () {
            sxd.ajax('index', sxd.lng('load'), {act: 'exit'});
        }
    },
    informer: function () {
        if (sxd.job.timer)clearTimeout(sxd.job.timer);
        if (sxd.job.stop)return;
        sxd.job.timer = setTimeout(function () {
            sxd.ajax('info', sxd.lng('run'), {job: sxd.job.name, act: 'info', lseek: sxd.job.log_seek}, 1, function () {
                sxd.informer();
            });
        }, 250);
    },
    addDb: function () {
        sxd.ajax('index', this.lng('sdb'), {
            act: 'add_db',
            name: z('db_name').value,
            charset: this.combos.db_charset.value,
            collate: this.combos.db_charset_col.value
        });
        sxd.hideDialog('createdb');
    },
    saveConnect: function () {
        sxd.ajax('index', this.lng('sc'), {
            act: 'save_connect',
        ...
        sxd.hideDialog('connect');
        z('con_pass').value = '';
    },
    job2php: function () {
        return sxd.opened == 'restore' || sxd.job.type == 'restore' ? {
            act: 'restore',
            type: 'restore',
    ...
    saveJob: function () {
        ...
        sxd.ajax('index', this.lng('sj'), t);
        sxd.hideDialog('savejob');
    },
    runSavedJob: function (type, job) {
        ...
        sxd.job.name = job;
        sxd.job.type = type;
        sxd.job.act = 'run_savedjob';
        sxd.ajax('index', this.lng('run'), {act: 'run_savedjob', type: type, job: job}, 1);
        this.openTab('log');
    ...
    saveOptions: function () {
        sxd.confirms = z('conf_import').checked * 1 + z('conf_file').checked * 2 + z('conf_db').checked * 4;
        sxd.ajax('index', this.lng('so'), {
            act: 'save_options',
    ...
    runBackup: function () {
        ...
        z('btn_pause').disabled = false;
        z('btn_again').disabled = true;
        sxd.job.name = this.newJob();
        sxd.job.type = 'backup';
        var t = this.job2php();
        this.openTab('log');
        t.type = 'run', t.job = sxd.job.name;
    ...
    stopJob: function () {
        ...
        sxd.ajax('info', this.lng('run'), {job: sxd.job.name, act: 'stop'}, 1);
    },
    pauseJob: function () {
        z('btn_again').disabled = false;
        z('btn_stop').disabled = true;
        z('btn_pause').disabled = true;
        sxd.ajax('info', this.lng('run'), {job: sxd.job.name, act: 'pause'}, 1);
    ...
    resumeJob: function () {
        ...
        z('btn_pause').disabled = false;
        z('btn_again').disabled = true;
        sxd.ajax('index', this.lng('run'), {job: sxd.job.name, act: 'resume'}, 1);
        z('btn_resume').style.display = 'none';
        z('btn_pause').style.display = '';
        sxd.job.stop = 0;
        sxd.informer();
    ...
    runAgain: function () {
        if (sxd.job.act == 'run_savedjob') {
            sxd.runSavedJob(sxd.job.type, sxd.job.name);
        }
        else if (sxd.job.type == 'backup') {
            sxd.runBackup();
        }
        else if (sxd.job.type == 'restore') {
            sxd.runRestore();
        }
    },
    runRestore: function () {
        if (sxd.confirms & 1 && !confirm(sxd.lng('fic')))return;
        this.clearLogTab();
        z('btn_resume').style.display = 'none';
        z('btn_pause').style.display = '';
        z('btn_down').style.display = 'none';
    ...
    runServices: function (type) {
        switch (type) {
            case'delete':
                if (sxd.confirms & 4 && !confirm(sxd.lng('ddc')))return;
                sxd.ajax('index', sxd.lng('run'), {act: 'delete_db', name: this.combos.services_db.value});
                return;
        }
        this.result.clear();
        this.openTab('result');
    ...
    runFiles: function (type, file) {
        ...
        switch (type) {
            case'open':
                sxd.combos.restore_file.select(file);
                sxd.openTab('restore');
                break;
            case'download':
                location.href = sxd.backupUrl + file;
                break;
            case'delete':
                if (!file)return;
                if (sxd.confirms & 2 && !confirm(sxd.lng('fdc')))return;
                sxd.ajax('index', sxd.lng('run'), {act: 'delete_file', name: file});
    ...
    addCombo: function (name, value, ico, opts) {
        ...
        c.select = function (value, wheel) {
            ...
            var _this = this;
            if (wheel) {
                clearTimeout(sxd.timeout);
                sxd.timeout = setTimeout(function () {
                    _this.action();
                }, 333);
            }
            else this.action();
    ...
    showMenu: function (obj, opts, cfg) {
        ...
        for (var o in opts) {
            ...
            s.onmousedown = function () {
                if (cfg.runjob)sxd.runSavedJob(m.obj.com, this.value); else if (cfg.btn)sxd.runServices(this.value); else m.obj.select(this.value, 0);
    ...
    formatSize: function (size, add) {
        ...
        if (size >= 0) {
            ...
            size = (i > 0 ? size.toPrecision(3) : size) + ' ' + sxdlng.sizes[i];
    ...
    return {
        init: function (elem, num) {
            ...
            for (var ico in treeIcons) {
                sxd.css.add('.zTree ' + treeIcons[ico] + ' {background-position: 0 -' + (ico * 18) + 'px;}');
            }
            var Icons = ['.cTA', '.cVI', '.cPR', '.cFU', '.cTR', '.cEV', '.iTA', '.iVI', '.iPR', '.iFU', '.iTR', '.iEV'];
            for (ico in Icons) {
                sxd.css.add('.zTree ' + Icons[ico] + ' {background-position: 0 -' + (ico * 16) + 'px;}');
            }
        }, getId: function (o) {
            ...
            var o = this.index[id];
            o.open = (o.open + 1) % 2;
            sxd.css.swap(o.el, ['close', 'open'], o.open);
        }, clickBranch: function (id) {
            if (sxd.version.type == 'lite' && this.name == 'restore_tree') {
                alert(sxd.lng('pro'));
                return false;
            }
            var o = this.index[id];
            ...
            o.el.getElementsByTagName('span')[0].innerHTML = sxd.formatSize(size, 0);
            return size;
        ...
        init: function (elem, buttons) {
            ...
            this.el.onmousedown = function (e) {
                ...
                if (_this.getId(e || window.event)) {
                    ...
                    if (_this.curBtn.more) {
                        ...
                        _this.curBtn.pop = 1;
                        sxd.showMenu(_this.curBtn, sxd.options['sj_' + _this.curBtn.com], {width: 220, runjob: true});
            ...
            this.el.onmouseup = function (e) {
                ...
                if (_this.getId(e || window.event)) {
                    ...
                    sxd.actions['tab_' + _this.curBtn.com](_this.curBtn.more);
                }
            };
            var icons = {
                '.btn.ovr': '0 -28px',
            ...
            for (var ico in icons) {
                sxd.css.add('.zTBar ' + ico + ' {background-position:' + icons[ico] + ';}');
            }
            var btn, sTBar = '';
...
function zProgressObject() {
    return {
        init: function (elem, type) {
            ...
            cur = cur || 0;
            tot = tot || 0;
            this.txt1.innerHTML = proc + '%' + (size ? ' ' + sxd.formatSize(size) : '') + (cur && tot ? ' [' + cur + '/' + tot + ']' : '');
            this.txt2.innerHTML = this.txt1.innerHTML;
            if (this.type == 2)document.title = proc + '% - ' + sxd.version.full;
...
function zGridObject() {
    return {
        init: function (elem, cfg) {
            ...
            var td;
            if (this.name == 'sxdGrid2')row[5] = sxd.formatSize(row[5], 2);
            for (var i = 0; i < this.cols; i++) {
                ...
                td.innerHTML = row[i] || '&nbsp;';
                td.title = row[i] || '';
                if (i == 0 && this.name == 'sxdGrid2')td.title += "\n" + row[7];
            }
